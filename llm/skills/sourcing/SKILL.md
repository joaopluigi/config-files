---
name: sourcing
description: "Source every fact from a real source instead of memory, for any question or task — an API's behavior, a project convention, requirements, prior art, a pull request's review comments. Work a precedence ladder, closest first: the task's own files and this repo's dependencies, then your org's own knowledge (its GitHub repos and pull-request discussions, plus internal docs in Confluence or Google Docs, Slack threads, and meeting notes and transcripts reached through the calendar, when connected), then the official docs, and only then all of public GitHub. Use whenever you need information you do not already have in front of you — to read, answer, look something up, check a convention, or gather requirements, not only before acting. Ships a gh_find.sh helper that turns GitHub code/repo/PR search into ready-to-cite URLs and reads a PR's discussion with bots filtered out."
